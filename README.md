...
- **One-click New Session** -- `+ New` button on the dashboard spawns a fresh `tmux new-session` with one of four tools pre-launched, laid out in a 2×2 grid (safe variants on top, YOLO variants below): **Claude** (`claude`), **Codex** (`codex`), **Claude YOLO** (`claude --dangerously-skip-permissions`), **omx YOLO** (`omx --madmax --xhigh`). Tooltips on each button show the exact command. Codex launches inherit the full OMX workflow context (`developer_instructions` + MCP servers + `model_reasoning_effort = "xhigh"`) from global `~/.codex/config.toml`, so bare `codex` already gets the same behavior as `omx` — no separate `omx` button needed. Click-through directory picker for cwd

...
| Codex MCP 审批窄终端不显示 waiting 徽章 | Dashboard 的 codex 卡片显示不 pending,但 tmux pane 明明有 `› 1. Allow` + `enter to submit` + MCP 审批标题 | 两个连锁 bug:(1) 窄终端 word-wrap 把标题断成 `Allow the X MCP server to run\ntool "Y"?` 两行,parser 的 `if phrase in lines[i]` 逐行匹配无法跨越 "run↔tool" 的换行,`"MCP server to run tool"` 这个 5 词 phrase 匹配失败 → 全盘 None;(2) 即便 phrase 匹配上,`_extract_codex_mcp_detail` 用 ` ` join 相邻行但**没 strip 前导空白**,结果 "run" 和 "tool" 之间变成 3 个空格,regex `Allow the (\S+) MCP server to run tool`(单空格)匹配不到 server 名,detail 退回到只有 tool 名。修复:question-phrase 扫描同时检查 `lines[i]` 和 `lines[i].strip() + " " + lines[i+1].strip()` 两种候选;`_extract_codex_mcp_detail` 改成 `" ".join(line.strip() for line in ...)`。回归 fixture 存在 `tests/fixtures/codex-mcp-approval-wrapped.txt`,跑 `uv run python tests/test_codex_parser.py` 验证 |
| 点 Approve 后卡片 UI 不完全清除 | Badge 和 Approve/Always 按钮消失,但 dot 还是黄呼吸灯 + "Always → ..." label 仍显示 + Waiting 计数器没减,只有刷新页面才清 | 前面修同签名连续审批的副作用:`approve_session` 清 DB + 设 suppress 之后,`periodic_pending_check` 的两个分支都不会再触发"pending cleared" broadcast(`if parsed:` 被 suppress 跳过,`elif db_tool:` 的 db_tool 已经是 None)。Optimistic JS 只补了 badge + buttons,剩下三项没人通知。修复三处:(1) `updateWaitingBadge` 清除态把 dot 从 `dot-waiting` 重置成 `dot-active`;(2) `approveSession` JS optimistic 路径加 `updateAlwaysLabel(sid, null)`;(3) server 端 `approve_session` 成功发 key 后自己 broadcast `{type: 'pending', pending_tool: null, waiting_count: ...}`,让 dashboard 和 Telegram-initiated 审批都走同一条 WS 通道。两路径幂等 |
